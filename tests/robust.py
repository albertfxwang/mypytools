...
class robust(object):
	...
	def loc_scale(self):
		return loc_scale(self.x, self.mu0, sig0=self.sig0, k=self.k, eps=self.eps)
	
	
def W_huber(y, k=1.5):
	# calculates the weight function
	W = where(abs(y)>1.5, k/abs(y), 1.0)
	return W
...
def location_huber(x, k=1.5, eps=0.01, sig=-1.0):
	...
	if sig <= 0:
		sig = median(abs(x - median(x)))  # use MADN as the initial estimate of scale
...
def loc_scale(x, mu0, sig0=-1., k=1.5, eps=0.01):
