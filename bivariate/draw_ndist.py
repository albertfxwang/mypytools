...
def draw_ndist(diskfrac,ntot,gtype,nout,nbins=arange(0.,9.),verbose=0):
   ...
   indexarray = arange(len(gtype))
   if verbose:
      print "In input, number of disks = ",sum(gtype==0)
      print "In input, number of devauc = ",sum(gtype==1)
   disk_indexarray = compress(gtype==0,indexarray)
   devauc_indexarray = compress(gtype==1,indexarray)
