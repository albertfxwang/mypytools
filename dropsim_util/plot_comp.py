...
"""
Plot the completenesses of the dropout simulations.
"""
...
def show_galfit_qf_comp2(kgrid1, kgrid2, c_drop1, c_drop2, chisqnulim=[0.4,5.0], drop='b', colors=['0.0','0.3']):
	...
	limits = limits1.copy(); limits[:,1] = limits2[:,1] # adopt the limit that encloses both fields
	print limits
	mbins = arange(limits[0][0], limits[0][1]+0.5, 0.5)
	rbins = arange(limits[1][0], limits[1][1]+0.5, 0.2)
	nbins = array([len(mbins)-1,len(rbins)-1]); print nbins
	...
	for k1 in kgrid1.kernels:
		...
		x0 = kern.x0_bin
		i1 = (x0[0] - limits[0,0]) / 0.5; i1 = int(round(i1))
		i2 = (x0[1] - limits[1,0]) / 0.2; i2 = int(round(i2))
	...
	mag_cull1 = compress((c1.reout>0)&in1d(c1.cullflag, [0,1,3,12]), c1.magout)
	re_cull1 = compress((c1.reout>0)&in1d(c1.cullflag, [0,1,3,12]), c1.reout)
	mag_cull2 = compress((c2.reout>0)&in1d(c2.cullflag, [0,1,3,12]), c2.magout)
	re_cull2 = compress((c2.reout>0)&in1d(c2.cullflag, [0,1,3,12]), c2.reout)
	ndrops_all1 = histogram2d(mag_cull1, log10(re_cull1), bins=[mbins,rbins])[0]
	ndrops_all2 = histogram2d(mag_cull2, log10(re_cull2), bins=[mbins,rbins])[0]
	mag1, re1, crit1 = fl.cleandata(c_drop1, chisqnulim=chisqnulim[0], limits=limits, drop=drop)
	mag2, re2, crit2 = fl.cleandata(c_drop2, chisqnulim=chisqnulim[1], limits=limits, drop=drop)
	...
	frac_kern_mbins1 = nkern_good_mbins1.astype('float') / nkern_input_mbins1.astype('float')
	nkern_input_mbins2 = maximum(nkern_input_ma2.sum(axis=1),1)
	nkern_good_mbins2 = nkern_good_ma2.sum(axis=1)
	frac_kern_mbins2 = nkern_good_mbins2.astype('float') / nkern_input_mbins2.astype('float')
	ndrops_all_mbins1 = maximum(ndrops_all_ma1.sum(axis=1), 1)
	ndrops_good_mbins1 = ndrops_good_ma1.sum(axis=1)
	frac_drops_mbins1 = ndrops_good_mbins1.astype('float') / ndrops_all_mbins1.astype('float')
	ndrops_all_mbins2 = maximum(ndrops_all_ma2.sum(axis=1), 1)
	ndrops_good_mbins2 = ndrops_good_ma2.sum(axis=1)
	frac_drops_mbins2 = ndrops_good_mbins2.astype('float') / ndrops_all_mbins2.astype('float')
	# Now start plotting
	fig = plt.figure(figsize=(8,12))
	plt.subplot(2,1,1)
	plt.plot(mbins[:-1], frac_kern_mbins1, ls='steps', label='simulations (GOODS)', c=colors[0], lw=3.0)
	plt.plot(mbins[:-1], frac_drops_mbins1, ls='steps--', label='sample (GOODS)', c=colors[0], lw=3.0)
	plt.plot(mbins[:-1], frac_kern_mbins2, ls='steps', label='simulations (HUDF)', c=colors[1], lw=1.5)
	plt.plot(mbins[:-1], frac_drops_mbins2, ls='steps--', label='sample (HUDF)', c=colors[1], lw=1.5)
	...
	frac_kern_rbins1 = nkern_good_rbins1.astype('float') / nkern_input_rbins1.astype('float')
	...
	frac_kern_rbins2 = nkern_good_rbins2.astype('float') / nkern_input_rbins2.astype('float')
	ndrops_all_rbins1 = maximum(ndrops_all_ma1.sum(axis=0), 1)
	ndrops_good_rbins1 = ndrops_good_ma1.sum(axis=0)
	frac_drops_rbins1 = ndrops_good_rbins1.astype('float') / ndrops_all_rbins1.astype('float')
	ndrops_all_rbins2 = maximum(ndrops_all_ma2.sum(axis=0), 1)
	ndrops_good_rbins2 = ndrops_good_ma2.sum(axis=0)
	frac_drops_rbins2 = ndrops_good_rbins2.astype('float') / ndrops_all_rbins2.astype('float')	
	plt.subplot(2,1,2)
	plt.plot(rbins[:-1], frac_kern_rbins1, ls='steps', label='simulations (GOODS)', c=colors[0], lw=3.0)
	plt.plot(rbins[:-1], frac_drops_rbins1, ls='steps--', label='sample (GOODS)', c=colors[0], lw=3.0)
	plt.plot(rbins[:-1], frac_kern_rbins2, ls='steps', label='simulations (HUDF)', c=colors[1], lw=1.5)
	plt.plot(rbins[:-1], frac_drops_rbins2, ls='steps--', label='sample (HUDF)', c=colors[1], lw=1.5)
	plt.xlim(limits[1][0], 1.6); plt.ylim(0.,1.)
	plt.legend(loc=3)
...
def showmap(marray, limits, mags=[], logres=[], bw=array([0.5,0.2]), pixdx=array([0.02,0.02]), 
   dtype='float', vrange=None, mask=None,
   newfig=True, fig=None, ax=None, cmap=cmap_hot, tickw=array([1.0, 0.4]), cbar_extend='neither'):
   # show the values of marray in each bin on a map, bins are determined by 
   # the limits and bin width
   nbins = (limits[:,1] - limits[:,0]) / bw
   nbins = nbins.astype('int')
   ...
   xticks = arange(0, npix[0], tickw[0]/pixdx[0]); xticks = xticks.astype('int')
   yticks = arange(0, npix[1], tickw[1]/pixdx[1]); yticks = yticks.astype('int')
   xtickvals = limits[0][0] + xticks * pixdx[0]
   ytickvals = limits[1][0] + yticks * pixdx[1]
