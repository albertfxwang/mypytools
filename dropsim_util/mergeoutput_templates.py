...
#def_inputcols = {'x_in':1, 'y_in':2, 'mag_in':3, 're_in':7}

def_inputcols_templates = {'x_in':1, 'y_in':2, 'mag_in':3, 'template':4, 'tempsize':5, 
	'ell_in':6, 'pa_in':7, 're_in':8}
...
def mergeoutput(dirs, root='run*m', bands=['H','B','V','I','Z','Y','J'],\
   inputband='H',inputcols_dic=def_inputcols_templates,inputcols_int=['gtype'],paramfile=None):
      ...
      for srun in subruns:
         ...
         for b in bands:
            ...
            if len(b) == 1:  # if detection is using a single band
               gallist[b] = sextractor(runroot+'_%s/gal%d_s.list' % (b, i))
...
def mergecat_txt(outcat, dirs, root='run*', bands=['H','B','V','I','Z','Y','J'],\
   inputband='H', inputcols_dic=def_inputcols_templates, inputcols_int=[], inputcols_str=[],
   paramfile=None):
