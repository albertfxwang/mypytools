...
class BdropsKernelFactory(KernelFactory):
   """
   For B-dropouts in GOODS + UDF.
   """
   ...
   def ColorSelection(self):
      ...
      print "Do selections..."
      color1 = self.acs_f435w_mag - self.acs_f606w_mag
      color2 = self.acs_f606w_mag - self.acs_f850lp_mag
      ...
      print "Total number selected as B-dropouts: %d" % (np.sum(self.lcc.crit))
...
class BdropoutKernelGrid(DropoutKernelGrid):
   """
   The top-level class that a user should call.
   """
   def __init__(self, field, kwargs_factory={}, kwargs_kgrid={}):
      Factory = BdropsKernelFactory(catalogs[field], field, **kwargs_factory)
      super(BdropoutKernelGrid, self).__init__(Factory, **kwargs_kgrid)

def make_udrops_kernel_grid(field, filename, kwargs_factory={'re_in':'re_in_arcsec'}, kwargs_kgrid={'ylimits':[-2.4, 1.0]}):
   """
   A convient function to do everything.
   The keyword arguments **kwargs are passed to UdropoutKernelGrid.__init__().
   """
   Grid = BdropoutKernelGrid(field, kwargs_factory, kwargs_kgrid)
   f = open(filename, 'wb')
