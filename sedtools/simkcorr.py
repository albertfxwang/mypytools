...
uni1500 = S.Box(1500., 100.)  # a boxcar filter around 1500A with width 100A
...
def simkcorr(sp, absmag_rest, restband, obsband, z, lya_ew):
   ...
   obsmag,spec_r,spec_at = MR.mag_redshift(spn,z,obsband,"abmag")
   if lya_ew > 0.:
      lyalam = 1216. * (1. + z)  # observed wavelength of Lya line
      spec_lya = simutil.add_lyman_alpha(spec_r, lyalam, lya_ew)
      obsmag = A.ABmag(spec_lya, obsband)
   else:
      obsmag = A.ABmag(spec_r, obsband)
